...
train_path = "Layer 2/L2_650x150/Train/*.csv"
test_path = "Layer 2/L2_650x150/Test/*.csv"
...
def tune_model(train,test):
    ...
    rf_grid = GridSearchCV(estimator= SVM, param_grid= parameters,cv=skf.get_n_splits(X_trainval, y_trainval), n_jobs=-1,scoring = 'neg_log_loss',return_train_score = True, iid = False)
...
a = tune_model(train,test)
y_test = np.array(tune_model(train,test)[1])
y_predictied = tune_model(train,test)[1]

train.iloc[:,0]
a = return_sum(2,3)[0]
    results = pd.DataFrame(rf_grid.cv_results_)
    results = results.T
    results.to_csv("RF_result" +str(n)+ ".csv", index = True)
...
df.to_csv('SVM_Score_neg_log_loss.csv',header=True)

df_pro.to_csv('SVM_Proba_neg_log_loss.csv', header= True)

results = pd.DataFrame(results)
results.head()
results = pd.DataFrame(results.T)
results.to_csv('results_SVM_neg_log_loss.csv',header = True)
